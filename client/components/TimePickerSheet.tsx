...
export default function TimePickerSheet({
  ...
  const draftIsValid = useMemo(
    () => (validateDraft ? validateDraft(draft) : true),
    [draft, validateDraft]
  );

  const triggerIsValid = useMemo(
    () => (validateDraft ? validateDraft(value) : true),
    [value, validateDraft]
  );
    ...
    <>
      <TouchableOpacity
        ...
        style={[
          styles.trigger,
          {
            borderColor: triggerIsValid ? borderColor : errorColor,
            backgroundColor: triggerIsValid ? cardColor : `${errorColor}10`,
          },
        ]}
      >
        <Text style={[styles.triggerText, { color: triggerIsValid ? textColor : errorColor }]}>{value}</Text>
      </TouchableOpacity>
        ...
        <View style={styles.overlay}>
          ...
          <View style={[styles.sheet, { backgroundColor: cardColor }]}>
            ...
            <TimeWheelPicker
              ...
              hasError={!draftIsValid}
              errorColor={errorColor}
            />
            {!draftIsValid && (
              <Text style={[styles.errorText, { color: errorColor }]}>
                Ten przedział koliduje z inną atrakcją lub ma nieprawidłowe godziny.
              </Text>
            )}
            <TouchableOpacity
              style={[
                styles.okButton,
                { backgroundColor: draftIsValid ? accentColor : errorColor, opacity: draftIsValid ? 1 : 0.85 },
              ]}
